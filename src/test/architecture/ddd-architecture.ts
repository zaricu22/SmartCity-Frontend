 ...
 *  7.  presentation must not import from domain directly
 ...
 * 19.  domain events (*Event classes) live only in domain/event
 ...
 *  B.  domain/repository → rxjs — Observable is Angular's async contract for repository
...
function getClassNames(file: string): string[] {
  const content = fs.readFileSync(file, 'utf8');
  return [...content.matchAll(/(?:export\s+)?(?:abstract\s+)?class\s+(\w+)/g)].map(m => m[1]);
}
...
function layerImports(sourceDir: string, targetDir: string): string[] {
  ...
  for (const file of getFiles(sourceDir)) {
    for (const imp of getImports(file)) {
      if (resolveImport(file, imp).startsWith(targetDir)) {
...
describe('Layer Dependencies', () => {
  ...
  rule('presentation must not import from domain directly',   layerImports(PRESENTATION,   DOMAIN));
