...
describe('EnergyDevice', () => {
  ...
  describe('constructor', () => {
    it('should create with valid arguments', () => {
      ...
      expect(device.name).toBe('Roof Panel');
    ...
    it('should throw when name is null', () => {
      let error: unknown;
      try { new EnergyDevice('id-1', null as any, DeviceType.SOLAR, capacity); } catch(e) { error = e; }
      expect(error).toBeInstanceOf(ValidationException);
      expect((error as ValidationException).errorCode).toBe(ErrorCode.DEVICE_NAME_EMPTY);
    });

    it('should throw when name is empty', () => {
      let error: unknown;
      try { new EnergyDevice('id-1', '', DeviceType.SOLAR, capacity); } catch(e) { error = e; }
      expect(error).toBeInstanceOf(ValidationException);
      expect((error as ValidationException).errorCode).toBe(ErrorCode.DEVICE_NAME_EMPTY);
    });
    ...
    it('should throw when type is null', () => {
      let error: unknown;
      try { new EnergyDevice('id-1', 'Roof Panel', null as any, capacity); } catch(e) { error = e; }
      expect(error).toBeInstanceOf(ValidationException);
    ...
    it('should throw when capacity is null', () => {
      let error: unknown;
      try { new EnergyDevice('id-1', 'Roof Panel', DeviceType.SOLAR, null as any); } catch(e) { error = e; }
      expect(error).toBeInstanceOf(ValidationException);
    ...
    it('should initialise production rate to 0 kW', () => {
      const device = new EnergyDevice('id-1', 'Backup Battery', DeviceType.BATTERY, capacity);
      expect(device.productionRate.value).toBe(0);
  ...
  describe('changeProduction()', () => {
    ...
    it('should throw when production in larger unit exceeds capacity', () => {
      const device = new EnergyDevice('id-1', 'Roof Panel', DeviceType.SOLAR, capacity); // 100 kW
  ...
  describe('equals()', () => {
    it('should be equal when ids match', () => {
      const a = new EnergyDevice('same-id', 'Roof Panel', DeviceType.SOLAR, capacity);
      const b = new EnergyDevice('same-id', 'Backup Battery', DeviceType.PUMP, new Energy(200, EnergyUnit.kW));
      expect(a.equals(b)).toBe(true);
    });

    it('should not be equal when ids differ', () => {
      const a = new EnergyDevice('id-1', 'Roof Panel', DeviceType.SOLAR, capacity);
      const b = new EnergyDevice('id-2', 'Roof Panel', DeviceType.SOLAR, capacity);
