...
| `GET` | `/v1/buildings?page=&size=&sort=&eligible=&name=&location=` | Paginated building list; `eligible=true` filters to subsidy-eligible buildings only (server-side, ADR-0024); `name`/`location` are case-insensitive "contains" filters combined with AND, not applied when `eligible=true` (ADR-0032) |
